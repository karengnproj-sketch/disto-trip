          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {liveHotels.map((h: any, i: number) => {
                ...
                const bookingSearchUrl = `https://www.booking.com/searchresults.html?ss=${encodeURIComponent(h.name + " Egypt")}`;
                const starCount = h.stars || (h.type === "hotel" ? 3 : h.type === "guest_house" ? 2 : 3);

                return (
                  <div key={h.id} className="group bg-[#1a1a1a] rounded-2xl border border-[#333]/50 overflow-hidden hover:border-[#39FF14]/30 hover:shadow-lg hover:shadow-[#39FF14]/5 transition-all duration-300">
                    {/* Image */}
                    <div className="relative aspect-[16/10] overflow-hidden">
                      <img src={img} alt={h.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                      <div className="absolute top-3 left-3 px-3 py-1 bg-black/70 backdrop-blur-sm rounded-full text-xs font-medium text-white">
                        {"*".repeat(starCount)} {h.type?.replace(/_/g, " ")}
                      </div>
                      <div className="absolute top-3 right-3 px-2 py-1 bg-blue-500/90 rounded-full text-[10px] font-bold text-white">
                        OSM
                      </div>
                    </div>
                    ...
                    <div className="p-5">
                      <h3 className="font-semibold text-white text-base leading-tight mb-2">
                        {isAr && h.name_ar ? h.name_ar : h.name}
                      </h3>

                      <div className="flex items-center gap-1 text-[#B0B0B0] text-xs mb-3">
                        <MapPin className="w-3 h-3" />
                        <span>{h.address || (isAr ? "مصر" : "Egypt")}</span>
                      </div>

                      {h.phone && (
                        <p className="text-[#888] text-xs mb-3">{h.phone}</p>
                      )}
                      ...
                      {/* Book Now */}
                      <a href={h.website || bookingSearchUrl} target="_blank" rel="noopener noreferrer"
                        className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-[#39FF14] to-[#00E676] text-black font-semibold rounded-xl hover:shadow-lg hover:shadow-[#39FF14]/25 hover:scale-[1.02] transition-all duration-300 text-sm">
                        {h.website ? (isAr ? "زيارة الموقع" : "Visit Website") : (isAr ? "ابحث على Booking.com" : "Search on Booking.com")} <ExternalLink className="w-3.5 h-3.5" />
                      </a>
                    </div>
                  </div>
                );
